...
path_model_era = '/Volumes/Elements2/ERA5-Land/model_data/'
# Path of model output SM
path_model_sm_gldas = '/Volumes/Elements2/VIIRS/SM_model_gldas/'
path_model_sm_era = '/Volumes/Elements2/VIIRS/SM_model_era/'
# Path of downscaled SM
path_smap_sm_ds_gldas = '/Volumes/Elements2/SMAP/SM_downscaled_gldas/'
path_smap_sm_ds_gldas_adjusted = '/Volumes/Elements2/SMAP/SM_downscaled_gldas_adjusted/'
path_smap_sm_ds_era = '/Volumes/Elements2/VIIRS/SM_downscaled_era/'
# Path of 9 km SMAP SM
path_smap = '/Volumes/Elements/Datasets/SMAP'
# Path of VIIRS data
path_viirs_lst = '/Volumes/Elements2/Dataset_UVA_data/VIIRS/LST/'
path_viirs_lst_filled = '/Volumes/Elements2/Dataset_UVA_data/VIIRS/LST_filled/'
path_viirs_missing = '/Volumes/Elements2/Dataset_UVA_data/VIIRS/Missing/'

# Path of VIIRS data regridded output
path_viirs_output = '/Users/binfang/Downloads/Processing/VIIRS/'
path_lst_geo = '/Volumes/Elements2/Dataset_UVA_data/VIIRS/LST_geo/'
path_lst_ease = '/Volumes/Elements2/Dataset_UVA_data/VIIRS/LST_ease/'
path_viirs_lai = '/Volumes/Seagate_6TB/VIIRS/LAI/'
path_lai_geo = '/Volumes/Elements2/Dataset_UVA_data/VIIRS/LAI_geo/'
path_lai_ease = '/Volumes/Elements2/Dataset_UVA_data/VIIRS/LAI_ease/'
...
varname_list = ['lat_world_max', 'lat_world_min', 'lon_world_max', 'lon_world_min',
                ...
                'row_world_ease_9km_from_400m_ind', 'col_world_ease_9km_from_400m_ind']
...
# Generate the world map of EASE-grid projection
# lat_world_ease_400m_group_center = [lat_world_ease_400m_group[x][2030] for x in range(len(lat_world_ease_400m_group))]
# lon_world_ease_400m_group_center = [lon_world_ease_400m_group[x][1807] for x in range(len(lon_world_ease_400m_group))]
# transformer = Transformer.from_crs("epsg:4326", "epsg:6933", always_xy=True)
# lon_world_ease_400m_meshgrid, lat_world_ease_400m_meshgrid = np.meshgrid(lon_world_ease_400m_group_center, lat_world_ease_400m_group_center)
# [lon_world_ease_400m_meter, lat_world_ease_400m_meter] = transformer.transform(lon_world_ease_400m_meshgrid, lat_world_ease_400m_meshgrid)
# df_coord_world_ease_400m = np.stack((lon_world_ease_400m_meter.ravel(), lat_world_ease_400m_meter.ravel()), axis=1)
# df_coord_world_ease_400m = pd.DataFrame(df_coord_world_ease_400m, columns=['x', 'y'])
# tile_name = ['T'+ str(x+1).zfill(3) for x in range(216)]
# df_tile_name = pd.DataFrame(tile_name, columns=['id'])
# df_coord_world_ease_400m = pd.concat([df_coord_world_ease_400m, df_tile_name], axis=1, sort=False)
# df_coord_world_ease_400m.to_csv('/Users/binfang/Downloads/df_coord_world_ease_400m.csv', mode='w')
...
for iyr in range(5, len(yearname)):
    ...
    # tile_name = [tile_name[x] for x in [22, 23, 24, 37, 38, 39, 52, 53]]
    tile_name_base = [tile_name[x].split('/')[-1].split('.')[0].split('_')[-2] for x in range(len(tile_name))]
    tile_name_ind = [int(tile_name[x].split('/')[-1].split('.')[0].split('_')[-2][1:])-1 for x in range(len(tile_name))]
    ...
    for ite in range(0, len(tile_name)):
        ...
        viirs_files = sorted(tar.getnames())
        # viirs_files.pop(2404)
        # viirs_files.pop(2558)
        ...
        for igp in range(0, len(overpass_time_ind_group_ind)):
            viirs_mat_all = []
            for ife in range(0, len(overpass_time_ind_group_ind[igp])):
                try:
                    gzip_file = tar.extractfile(viirs_files[overpass_time_ind_group_ind[igp][ife]])

                    # Skip if extraction failed
                    if gzip_file is None:
                        # print(f"Skipping missing file: {viirs_files[file_index]}")
                        continue
...
for iyr in [12]:#range(len(yearname)):
    tile_name = sorted(glob.glob(path_lst_geo + str(yearname[iyr]) + '/*/', recursive=True))
    ...
    for irow in range(9):
        for icol in range(24):
            ...
            tiles_load_num = [int(tiles_load[x]) for x in range(len(tiles_load))]
            tiles_in_result = any(elem in tiles_load_num for elem in tile_name_base)

            if tiles_in_result == True:
...
for iyr in [12]:#range(len(yearname)):
    file_name = sorted(glob.glob(path_viirs_lai + str(yearname[iyr]) + '/*'))
...
os.chdir(path_model_era)
f = h5py.File("ds_model_coef_era.hdf5", "r")
...
# ERA5-land 9 km model
for iyr in [10]:#range(len(yearname)):

    tile_name = sorted(glob.glob(path_lai_ease + str(yearname[iyr]) + '/*/', recursive=True))
    tile_name_base = [int(tile_name[x].split('/')[-2][1:]) for x in range(len(tile_name))]
    doy_month_ind = np.concatenate([np.repeat(x+1, daysofmonth_seq[x, iyr]) for x in range(0, 12)])
    ...
    for irow in range(9):
        for icol in range(24):
            ...
            if tile_num in tile_name_base:
                ...
                coef_mat_am_sub_all = [coef_mat_am[x][row_meshgrid_from_9km, col_meshgrid_from_9km, :].squeeze()
                                            for x in range(len(coef_mat_am))]
                coef_mat_pm_sub_all = [coef_mat_pm[x][row_meshgrid_from_9km, col_meshgrid_from_9km, :].squeeze()
                ...
                for idt in range(len(viirs_lst_file_list)):
                    ...
                    viirs_lai_mat_scale = f_lai[lai_varname_list[0]][()].ravel()
                    viirs_lai_mat_scale[np.isnan(viirs_lai_mat_scale)] = 0
                    viirs_lai_mat_scale = np.fix(viirs_lai_mat_scale).astype(int)
                    viirs_lai_mat_scale[viirs_lai_mat_scale >= 5] = 5
                    f_lai.close()
                    ...
                    model_sm_400m_am[model_sm_400m_am < 0] = np.nan
                    ...
                    model_sm_400m_pm[model_sm_400m_pm < 0] = np.nan
                    model_sm_400m_pm = np.reshape(model_sm_400m_pm, (4060, 3615))

                    # Write to file
                    model_sm_tile_output_path = path_model_sm_era + str(yearname[iyr]) + '/T' + str(tile_num).zfill(3) + '/'
                ...
                del(row_world_ease_400m_from_9km_ind_sub, col_world_ease_400m_from_9km_ind_sub, row_meshgrid_from_9km,
                    col_meshgrid_from_9km, coef_mat_am_sub_all, coef_mat_pm_sub_all)
            ...
            if tile_num in tile_name_base:
                ...
                for idt in range(len(viirs_lst_file_list)):
                    ...
                    viirs_lai_mat_scale = ((viirs_lai_mat - np.nanmin(viirs_lai_mat))/
                                           (np.nanmax(viirs_lai_mat) - np.nanmin(viirs_lai_mat))) # For scaling from LAI to NDVI
                    viirs_lai_mat_scale[np.isnan(viirs_lai_mat_scale)] = 0
                    ...
                    # Write to file
                    model_sm_tile_output_path = path_model_sm_gldas + str(yearname[iyr]) + '/T' + str(tile_num).zfill(3) + '/'
                    model_sm_file_output_path = ('model_sm_400m_' + str(yearname[iyr]) + str(idt+1).zfill(3) + '_T'
                                                 + str(tile_num).zfill(3) + '.hdf5')
...
for iyr in [12]:#range(len(yearname)):
    ...
    tile_name = sorted(glob.glob(path_model_sm_gldas + str(yearname[iyr]) + '/*/', recursive=True))
    tile_name_base = [int(tile_name[x].split('/')[-2][1:]) for x in range(len(tile_name))]
    doy_month_ind = np.concatenate([np.repeat(x+1, daysofmonth_seq[x, iyr]) for x in range(0, 12)])
            ...
            if tile_num in tile_name_base:
                ...
                for idt in range(daysofyear[iyr]):
                    ...
                    for ife in range(len(tiles_load)):
                        model_sm_file = (path_model_sm_gldas + str(yearname[iyr]) + '/T' + str(tiles_load[ife]).zfill(3) + '/'
                    ...
                    # Create output path
                    smap_sm_ds_tile_output_path = path_smap_sm_ds_gldas + str(yearname[iyr]) + '/T' + str(tile_num).zfill(3) + '/'
...
########################################################################################################################
# 5. Local Ratio Correction with Adaptive Weighting
...
# Col#1: 400m indices, Col#2: 9km indices
row_world_ease_400m_ind_tile = np.repeat(np.arange(9), 4060)
row_world_ease_9km_from_400m_ind_tile = [np.repeat(x, len(row_world_ease_9km_from_400m_ind[x]))
                                                 for x in range(len(row_world_ease_9km_from_400m_ind))]
row_world_ease_9km_from_400m_ind_tile = np.array(list(itertools.chain(*row_world_ease_9km_from_400m_ind_tile)))
row_world_ease_9km_from_400m_ind_flat = np.array(list(itertools.chain(*row_world_ease_9km_from_400m_ind.tolist())))
row_world_ease_9km_from_400m_ind_flat = np.stack((row_world_ease_9km_from_400m_ind_flat,
                                                                row_world_ease_9km_from_400m_ind_tile), axis=1)

col_world_ease_400m_ind_tile = np.repeat(np.arange(24), 3615)
col_world_ease_9km_from_400m_ind_tile = [np.repeat(x, len(col_world_ease_9km_from_400m_ind[x]))
                                                 for x in range(len(col_world_ease_9km_from_400m_ind))]
col_world_ease_9km_from_400m_ind_tile = np.array(list(itertools.chain(*col_world_ease_9km_from_400m_ind_tile)))
col_world_ease_9km_from_400m_ind_flat = np.array(list(itertools.chain(*col_world_ease_9km_from_400m_ind.tolist())))
col_world_ease_9km_from_400m_ind_flat = np.stack((col_world_ease_9km_from_400m_ind_flat,
                                                                col_world_ease_9km_from_400m_ind_tile), axis=1)
...
for iyr in range(len(yearname)):
    ...
    for irow in range(9):
        for icol in range(24):
            ...
            if tile_num in tile_name_base:
                # Find start/end row/col indices
                row_start_9km = row_world_ease_9km_from_400m_ind_flat[
                    row_world_ease_9km_from_400m_ind_flat[:, 0] == row_world_ease_400m_ind_group[irow][0]][0, 1]
                row_end_9km = row_world_ease_9km_from_400m_ind_flat[
                    row_world_ease_9km_from_400m_ind_flat[:, 0] == row_world_ease_400m_ind_group[irow][-1]][-1, 1]
                row_agg_9km_start = row_world_ease_9km_from_400m_ind[row_start_9km]
                row_agg_9km_end = row_world_ease_9km_from_400m_ind[row_end_9km]
                row_agg_9km_start_ind = row_agg_9km_start[0]
                row_agg_9km_start_tile = row_world_ease_400m_ind_tile[row_agg_9km_start_ind]
                row_agg_9km_end_ind = row_agg_9km_end[-1]
                row_agg_9km_end_tile = row_world_ease_400m_ind_tile[row_agg_9km_end_ind]
                ...
                row_first_tile_ind = row_world_ease_400m_ind_group[row_agg_9km_start_tile][0]
                row_diff_tofirst = row_agg_9km_start_ind - row_first_tile_ind
                row_agg_9km_local = row_world_ease_9km_from_400m_ind[row_start_9km:row_end_9km+1] - row_agg_9km_start_ind
                row_end_tile_ind = row_world_ease_400m_ind_group[row_agg_9km_end_tile][-1]
                row_diff_toend = row_agg_9km_end_ind - row_first_tile_ind
                row_400m_sub = row_world_ease_400m_ind_group[irow][0] - row_agg_9km_start_ind

                col_start_9km = col_world_ease_9km_from_400m_ind_flat[
                    col_world_ease_9km_from_400m_ind_flat[:, 0] == col_world_ease_400m_ind_group[icol][0]][0, 1]
                col_end_9km = col_world_ease_9km_from_400m_ind_flat[
                    col_world_ease_9km_from_400m_ind_flat[:, 0] == col_world_ease_400m_ind_group[icol][-1]][-1, 1]
                col_agg_9km_start = col_world_ease_9km_from_400m_ind[col_start_9km]
                col_agg_9km_end = col_world_ease_9km_from_400m_ind[col_end_9km]
                col_agg_9km_start_ind = col_agg_9km_start[0]
                col_agg_9km_start_tile = col_world_ease_400m_ind_tile[col_agg_9km_start_ind]
                col_agg_9km_end_ind = col_agg_9km_end[-1]
                col_agg_9km_end_tile = col_world_ease_400m_ind_tile[col_agg_9km_end_ind]
                ...
                col_first_tile_ind = col_world_ease_400m_ind_group[col_agg_9km_start_tile][0]
                col_diff_tofirst = col_agg_9km_start_ind - col_first_tile_ind
                col_agg_9km_local = col_world_ease_9km_from_400m_ind[col_start_9km:col_end_9km+1] - col_agg_9km_start_ind
                col_end_tile_ind = col_world_ease_400m_ind_group[col_agg_9km_end_tile][-1]
                col_diff_toend = col_agg_9km_end_ind - col_first_tile_ind
                col_400m_sub = col_world_ease_400m_ind_group[icol][0] - col_agg_9km_start_ind
                ...
                for idt in range(daysofyear[iyr]):
                    ...
                    f_smap = h5py.File(smap_sm_file, "r")
                    varname_list = list(f_smap.keys())
                    smap_sm_9km_am = f_smap[varname_list[0]][row_world_ease_400m_from_9km_ind_sub[0]:row_world_ease_400m_from_9km_ind_sub[-1]+1,
                                     col_world_ease_400m_from_9km_ind_sub[0]:col_world_ease_400m_from_9km_ind_sub[-1]+1, int(date_read[6:])-1]
                    smap_sm_9km_pm = f_smap[varname_list[1]][row_world_ease_400m_from_9km_ind_sub[0]:row_world_ease_400m_from_9km_ind_sub[-1]+1,
                                     col_world_ease_400m_from_9km_ind_sub[0]:col_world_ease_400m_from_9km_ind_sub[-1]+1, int(date_read[6:])-1]
                    f_smap.close()


                    # Original downscaled 400m SM
                    smap_sm_file = (path_smap_sm_ds_gldas + str(yearname[iyr]) + '/T' + str(tile_num).zfill(3) + '/'
                                          + 'smap_sm_400m_' + str(yearname[iyr]) + str(idt + 1).zfill(3) + '_T'
                                          + str(tile_num).zfill(3) + '.tif')

                    if os.path.exists(smap_sm_file) == True:
                        smap_sm_400m_am_sub = rasterio.open(smap_sm_file).read()[0, :, :]
                        smap_sm_400m_pm_sub = rasterio.open(smap_sm_file).read()[1, :, :]
                    else:
                        smap_sm_400m_am_sub = smap_sm_mat_fill
                        smap_sm_400m_pm_sub = smap_sm_mat_fill


                    # Adjust 400 m downscaled AM/PM SM
                    # Define common smoothing function
                    def nan_gaussian_smooth(img, sigma=2):
                        valid_mask = ~np.isnan(img)
                        img_zeroed = np.where(valid_mask, img, 0.0)
                        weights = valid_mask.astype(float)

                        smooth_sum = gaussian_filter(img_zeroed, sigma=sigma)
                        smooth_weights = gaussian_filter(weights, sigma=sigma)

                        with np.errstate(invalid='ignore', divide='ignore'):
                            result = np.where(smooth_weights > 0, smooth_sum / smooth_weights, np.nan)
                        return result


                    # Function for one-pass correction process
                    def correct_sm(fine, coarse):
                        fine = fine.copy()
                        coarse = coarse.copy()
                        fine_h, fine_w = fine.shape
                        coarse_h, coarse_w = coarse.shape
                        factor_h, factor_w = fine_h // coarse_h, fine_w // coarse_w

                        # -- Step 1: Block-wise NaN-aware mean
                        mask = ~np.isnan(fine)
                        sum_blocks = block_reduce(np.nan_to_num(fine), (factor_h, factor_w), func=np.sum)
                        count_blocks = block_reduce(mask.astype(float), (factor_h, factor_w), func=np.sum)
                        with np.errstate(divide='ignore', invalid='ignore'):
                            local_mean_400m = np.where(count_blocks > 0, sum_blocks / count_blocks, np.nan)

                        # -- Step 2: Upsample coarse and mean maps
                        coarse_up = resize(coarse, (fine_h, fine_w), order=1, preserve_range=True, anti_aliasing=False)
                        local_mean_up = resize(local_mean_400m, (fine_h, fine_w), order=1, preserve_range=True,
                                               anti_aliasing=False)

                        # -- Step 3: Smooth both upsampled layers
                        coarse_up_smooth = nan_gaussian_smooth(coarse_up, sigma=2)
                        local_mean_up_smooth = nan_gaussian_smooth(local_mean_up, sigma=2)

                        # -- Step 4: Ratio map
                        epsilon = 1e-6
                        valid_mask = ~np.isnan(fine)
                        ratio = np.full_like(fine, np.nan)
                        ratio[valid_mask] = fine[valid_mask] / (local_mean_up_smooth[valid_mask] + epsilon)

                        # -- Step 5: Smooth ratio
                        ratio_smooth = nan_gaussian_smooth(ratio, sigma=1)

                        # -- Step 6: Apply correction
                        adjusted = np.full_like(fine, np.nan)
                        adjusted[valid_mask] = ratio_smooth[valid_mask] * coarse_up_smooth[valid_mask]

                        # -- Step 7: Clip to valid SM range
                        adjusted = np.clip(adjusted, 0, 0.5)

                        return adjusted


                    # Run for AM and PM maps
                    adjusted_sm_400m_am = correct_sm(smap_sm_400m_am_sub, smap_sm_9km_am)
                    adjusted_sm_400m_pm = correct_sm(smap_sm_400m_pm_sub, smap_sm_9km_pm)

                    smap_sm_400m_ds = list((adjusted_sm_400m_am, adjusted_sm_400m_pm))

                    # Save the daily downscaled 400 m SM to Geotiff files
                    # Create output path
                    smap_sm_ds_tile_output_path = path_smap_sm_ds_gldas_adjusted + str(yearname[iyr]) + '/T' + str(tile_num).zfill(3) + '/'
                    smap_sm_ds_file_output_path = ('smap_sm_400m_' + str(yearname[iyr]) + str(idt + 1).zfill(3) + '_T'
                                                 + str(tile_num).zfill(3) + '.tif')
                    ...
                    print(smap_sm_ds_file_output_path)
                    del(smap_sm_400m_ds, adjusted_sm_400m_am, adjusted_sm_400m_pm, out_ds_tiff, gts)
                    del(smap_sm_400m_am_sub, smap_sm_400m_pm_sub)
